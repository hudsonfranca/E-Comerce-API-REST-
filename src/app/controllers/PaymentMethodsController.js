    ...
    async index(req,res){
        ...
        try{
            ...
            const response  =  await sequelize.transaction(async(t)=>{
                const allPaymentMethods = await payment_methods.findAll({transaction:t});
            ...
            return  res.status(400).json({error:"Unable to return all payment methods ."});
    ...
    async update(req,res){
        ...
        try{
            const response  =  await sequelize.transaction(async(t)=>{
                ...
                const [lines,updatedPaymentMethods] = await payment_methods.update(req.body,{
                    where: { id:findpaymentMethods.id },
                    returning: true,
                    transaction:t
                  });
